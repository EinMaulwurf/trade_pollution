...
data_pollution[[1]] <- terra::rast("./data//pollution data/V5GL02.HybridPM25.China.200001-200012.nc")
data_pollution[[2]] <- terra::rast("./data//pollution data/V5GL02.HybridPM25.China.200101-200112.nc")
data_pollution[[3]] <- terra::rast("./data//pollution data/V5GL02.HybridPM25.China.200201-200212.nc")
data_pollution[[4]] <- terra::rast("./data//pollution data/V5GL02.HybridPM25.China.200301-200312.nc")
data_pollution[[5]] <- terra::rast("./data//pollution data/V5GL02.HybridPM25.China.200401-200412.nc")
data_pollution[[6]] <- terra::rast("./data//pollution data/V5GL02.HybridPM25.China.200501-200512.nc")
data_pollution[[7]] <- terra::rast("./data//pollution data/V5GL02.HybridPM25.China.200601-200612.nc")

# Drop units, crop and combine with wind data
data_wind_pollution <- map(data_pollution, function(x) {
  terra::units(x) <- NA
...
```{r}
data_firm_wind_pollution <- map2(data_wind_pollution, 
                                 data_firm_list, 
                                 function(data_wind_pollution, data_firm_list) {
  terra::extract(data_wind_pollution, data_firm_list, xy = TRUE) %>%
    cbind(firm_code = data_firm_list$firm_code,
          year = data_firm_list$year) %>%
    drop_na() %>%
    st_as_sf(coords = c("x", "y"), crs = 4326)
                                   }, .progress = TRUE) %>%
  map_dfr(identity)
