...
data_firm <- list.files(path = "./data/firm data", full.names = TRUE) %>%
  read_csv() %>%
  drop_na() %>%
  st_as_sf(coords = c("lng_wgs84", "lat_wgs84"), crs = 4326)

## TODO: Adding year variable when importing

data_pollution <- terra::rast("./data//pollution data/V5GL02.HybridPM25.China.200001-200012.nc")
...
data_firm_wind_pollution_surrounding <- data_firm_wind_pollution %>%
  ...
  tibble() %>%
  st_drop_geometry() %>%
  select(firm_code, GWRPM25_surrounding_mean = surrounding_GWRPM25_average, GWRPM25_surrounding_sum = surrounding_GWRPM25_sum)
...
```{r}
data_firm_wind_pollution_surrounding %>%
  select(firm_code, surrounding_GWRPM25_average, surrounding_GWRPM25_sum) %>%
  write_csv("./data/data_firm_wind_pollution_surrounding.csv")
```
...
for (i in 1:num_batches) {
  ...
  batch_result <- batch %>%
    ...
    st_drop_geometry() %>%
    select(firm_code, GWRPM25_mean)
  
  result[[i]] <- batch_result
}

# Combine the results into a single data frame
#data_firm_wind_pollution_cone_average <- do.call(rbind, result)

# Using purrr
# Extract specific columns from each dataframe in the list
result_subset <- map(result, ~.x %>%
                             st_drop_geometry() %>%
                             select(firm_code, GWRPM25_mean))
# Combine the subsetted dataframes into a single dataframe
data_firm_wind_pollution_cone_average <- map_dfr(result_subset, identity)
```
...
Save as CSV file
```{r}
data_firm_wind_pollution_cone_average %>%
  select(firm_code = data_firm.firm_code, mean_GWRPM25) %>%
  write_csv("./data/data_firm_wind_pollution_cone_average.csv")
```
...
Save as CSV
```{r}
data_firm_wind_pollution_cone_windspeed %>%
  st_drop_geometry() %>%
  select(firm_code = data_firm.firm_code, GWRPM25_mean) %>%
  write_csv("./data/data_firm_wind_pollution_cone_windspeed.csv")
```
...
data_firm_wind_pollution <- read_rds("./data/data_firm_wind_pollution.rds") %>%
  st_drop_geometry() %>%
  select(firm_code, wind_speed, wind_dir, GWRPM25)
...
data_firm_wind_pollution %>%
  left_join(data_firm_wind_pollution_surrounding, by = "firm_code") %>%
  left_join(data_firm_wind_pollution_cone_average, by = "firm_code") %>%
  left_join(data_firm_wind_pollution_cone_sum, by = "firm_code") %>%
  left_join(data_firm_wind_pollution_cone_average_windspeed, by = "firm_code") %>%
  left_join(data_firm_wind_pollution_cone_sum_windspeed, by = "firm_code")
```

PROBLEM: firm_code is not unique over years, so I have to add a year variable when importing the data.