...
```{r}
#writeCDF(grid_small_final, "grid_small_final.nc", overwrite = TRUE)
...
#### Stack overflow
From [stack overflow](https://stackoverflow.com/questions/78119309/how-to-create-a-cone-polygon-to-sample-spatraster-to-represent-a-cameras-field-o):
```{r}
...
tic()
test <- test %>%
  mutate(mean_GWRPM25 = cone_average_func(geom_cone))
toc()
#ggplot()+
#  geom_sf(aes(geometry = geometry))+
#  geom_sf(aes(geometry = geom_cone))
...
```{r, warning=FALSE}
# Example why raw sum is a bad idea: The number of cells taken into account for calculation varies by the angle of the cone.
...
cone_example <- cone_func(point = firm_example$geometry, angle = firm_example$wind_dir)
extraction_example <- terra::extract(data_wind_pollution$GWRPM25, vect(cone_example), xy = TRUE) %>%
  st_as_sf(coords = c("x", "y")) %>%
  st_set_crs(4326)
...
for(i in 1:nrow(data_firm_wind_pollution)) {
  ...
  cone_i <- cone_func(point = data_firm_wind_pollution[i, "geometry"], angle = st_drop_geometry(data_firm_wind_pollution[i, "wind_dir"]))
  mean_GWRPM25_vec[i] <- cone_average_func(cone_i)
}
