...
data_firm <- read_csv("./data//firm data/m2000_firmcoordinates_wgs84.csv") %>%
...
```{r}
# Save
write_rds(data_firm_wind_pollution_surrounding, "./data/data_firm_wind_pollution_surrounding.rds")
# Restore
...
```{r}
data_firm_wind_pollution_surrounding %>%
  select(firm_code = data_firm.firm_code, surrounding_GWRPM25_average, surrounding_GWRPM25_sum) %>%
...
The following takes very, very long to run (8h+ on my server)!
```{r}
# Setting up for-loop
...
```{r}
# Save
saveRDS(data_firm_wind_pollution_cone, file = "./data/data_firm_wind_pollution_cone.rds")
# Restore
data_firm_wind_pollution_cone <- readRDS(file = "./data/data_firm_wind_pollution_cone.rds")
```
...
Save as CSV file
```{r}
data_firm_wind_pollution_cone %>%
  select(firm_code = data_firm.firm_code, mean_GWRPM25) %>%
  write_csv("./data/data_firm_wind_pollution_cone.csv")
```
...
The following approach uses too much RAM (difference is that now some cones are much larger) and crashes my system.
```{r}
...
Idea: Split into smaller batches to process.
```{r}
# Find median overall windspeed
# QUESTION: Median windspeed for whole raster or median windspeed for firms?
# ASSUMPTION: Median windspeed over whole raster
...
# Setting up batches
batch_size <- 1000
batches <- split(data_firm_wind_pollution, ceiling(seq_along(data_firm_wind_pollution$geometry) / batch_size))
...
data_firm_wind_pollution_cone_windspeed <- do.call(rbind, result)
```
(Took 15 min)
...
```{r}
# Save
saveRDS(data_firm_wind_pollution_cone_windspeed, file = "./data/data_firm_wind_pollution_cone_windspeed.rds")

# Restore
data_firm_wind_pollution_cone_windspeed <- read_rds("./data/data_firm_wind_pollution_cone_windspeed.rds")
```
