...
# Define new cone average func that now also takes in the raster as argument
cone_average_func_test <- function(cone, point, raster) {
  point_df <- tibble(geom_point = point) %>%
    mutate(ID = 1:nrow(.)) %>%
    st_as_sf()
...
# Define function that makes all the calculations and calls the other functions
test_function <- function(data_wind_pollution, data_firm_wind_pollution){
    # Setting up batches
    batch_size <- 1000
    num_batches <- ceiling(nrow(data_firm_wind_pollution) / batch_size)
    ...
    batch_result <- batch %>%
      ...
      mutate(geom_cone = cone_func(point = geometry, angle = wind_dir)) %>%
      ungroup() %>%
      mutate(GWRPM25_mean = cone_average_func_test(cone = geom_cone, point = geometry, 
...
# Creating small sample for testing
data_firm_list_test <- data_firm_list %>% map(~.x %>% slice_sample(n = 2000))
data_firm_wind_pollution_list_test <- data_firm_wind_pollution_list %>% map(~.x %>% slice_sample(n = 2000))
...
test <- map2(data_wind_pollution_list,
     data_firm_wind_pollution_list_test, 
     test_function, .progress = TRUE)
